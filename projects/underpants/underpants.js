...
_.first = function(array, n) {
    //test if not array return []//
    // if n undefned or naN return array [0]
    // otherwise return first n nmber of elements
    if(!Array.isArray(array)|| 0 > n) return [];
...
_.last = function(array, n){
    // test if not array return []
    // if undefined or nan return [array.length-1]
    //otherwise return the last number in array
    if(!Array.isArray(array) || 0 > n) return [];
...
_.each = function(collection, action) {
    //What is kind of collection is it? array call function once 
    //for each arguement
    //If object call fuction once for each property with arguments
